			       ...
			       uint8_t *nhlt_buffer, uint32_t nhlt_size)
{
	...
	/*ret = snd_config_merge(data, cfg_new, true);*/
...
static int manifest_create(snd_config_t *input, uint8_t **manifest_buffer, uint32_t *size, uint32_t nhlt_size)
{
	...
	if (strlen(abi) != SOF_ABI_CHARS) {
		manifest.abi_major = 0;
		manifest.abi_minor = 0;
		manifest.abi_patch = 0;
	} else {
		sscanf(abi, "%" SCNx8, &manifest.abi_major);
		sscanf(&abi[ALSA_BYTE_CHARS], "%" SCNx8, &manifest.abi_minor);
		sscanf(&abi[ALSA_BYTE_CHARS * 2], "%" SCNx8, &manifest.abi_patch);
	}
	...
	*size = manifest_size;
	...
	top_p = (uint8_t *)&manifest;
	dst = byte_buffer;
	for (i = 0; i < sizeof(struct sof_manifest); i++)
		*dst++ = *top_p++;
