...
static int configure_registers(struct intel_dmic_params *dmic, struct dmic_calc_configuration *cfg)
{
	...
	if (dmic->dmic_prm[di].driver_version == 1 ||
	    dmic->dmic_prm[di].driver_version == 2) {
		if (di == 0) {
	...
	if (dmic->dmic_prm[di].driver_version == 3) {
		if (di == 0) {
...
static struct intel_dmic_params *get_dmic(struct tplg_pre_processor *tplg_pp)
{
	...
	dmic = (struct intel_dmic_params *)nhlt->dmic_params;
	if (!dmic)
		return NULL;
 ...
 * sensitivity.
 */
int dmic_calculate(struct tplg_pre_processor *tplg_pp)
{
...
int dmic_get_params(struct tplg_pre_processor *tplg_pp, int index, uint32_t *sample_rate,
...
int dmic_get_mic_params(struct tplg_pre_processor *tplg_pp, int index,
...
int dmic_get_vendor_blob_size(struct tplg_pre_processor *tplg_pp, size_t *size)
{
	struct intel_dmic_params *dmic;
...
int dmic_get_vendor_blob_count(struct tplg_pre_processor *tplg_pp)
{
	struct intel_dmic_params *dmic;
...
int dmic_get_vendor_blob(struct tplg_pre_processor *tplg_pp, uint8_t *vendor_blob)
{
	struct intel_dmic_params *dmic;
	int i, fir_index_0, fir_index_1;
	...
	dmic_get_vendor_blob_size(tplg_pp, &blob_size);
...
int dmic_set_params(struct tplg_pre_processor *tplg_pp, int dai_index, int driver_version,
...
int dmic_set_pdm_params(struct tplg_pre_processor *tplg_pp, int pdm_index, int enable_a,
...
int dmic_set_ext_params(struct tplg_pre_processor *tplg_pp, uint32_t snr, uint32_t sensitivity)
{
	struct intel_dmic_params *dmic;
...
int dmic_set_mic_params(struct tplg_pre_processor *tplg_pp, int index,
			uint8_t type, uint8_t panel, uint32_t speaker_position_distance,
			uint32_t horizontal_offset, uint32_t vertical_offset,
			uint8_t frequency_low_band, uint8_t frequency_high_band,
			uint16_t direction_angle, uint16_t elevation_angle,
...
int dmic_init_params(struct tplg_pre_processor *tplg_pp)
{
	struct intel_nhlt_params *nhlt;
	struct intel_dmic_params *dmic;
	int i;
	...
	dmic->dmic_mic_config.array_type = 0xA;
